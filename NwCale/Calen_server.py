...
def remove_event():
    ...
    prevEvent = request.form['Event4Eli']
    conn = sqlite3.connect('events.db')
    cursor = conn.cursor()
    print("date", date)
    ...
    print("Event4Delet::--", prevEvent)
    userIdyp="sahrif"
    #print("event_data", event_data)
    # Insert event into the database
    cursor.execute("DELETE FROM events WHERE date = ? AND event = ? AND username = ?", (nuova_datay,prevEvent, userIdy))
    conn.commit()
